      ...
      val mat =
        {
          val comp =
            if (f == scaladsl.Keep.left) {
              if (IgnorableMatValComp(matComputation2)) matComputation1 else null
            } else if (f == scaladsl.Keep.right) {
              if (IgnorableMatValComp(matComputation1)) matComputation2 else null
            } else null
          if (comp == null) Combine(f.asInstanceOf[(Any, Any) ⇒ Any], matComputation1, matComputation2)
          else comp
        }
